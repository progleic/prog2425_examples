...
template <typename T>
simple_vector<T>::simple_vector(int initial_capacity) : capacity_(initial_capacity), size_(0)
{
  elements_ = new T[capacity_];
}

template <typename T>
simple_vector<T>::simple_vector(const simple_vector<T> &sv) : capacity_(sv.capacity_), size_(sv.size_)
{
  elements_ = new T[capacity_];
